...
uploadRoutes.post('/upload', uploadZip.single('gitZip'), async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!repoPath) {
      res.status(400).json({ error: 'No valid git repository found in the uploaded zip' });
      return;
    }
    ...
    if (!isValid) {
      res.status(400).json({ error: 'The uploaded content is not a valid git repository' });
      return;
    }
...
uploadRoutes.post('/upload-folder', uploadFolder.array('files'), async (req: Request, res: Response) => {
  let tempDir: string | null = null;
  ...
  try {
    ...
    if (!repoPath) {
      res.status(400).json({ error: 'No .git folder found in the selected directory' });
      return;
    }
    ...
    const isValid = await gitService.validateRepository(repoPath);
    if (!isValid) {
      res.status(400).json({ error: 'The selected folder is not a valid git repository' });
      return;
    }
